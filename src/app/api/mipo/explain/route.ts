...
import type { Size } from "@/src/domain/commerce";
import type { AgentAnswer } from "@/src/domain/agent";

type Intervention = {id:string;session_id:string;product_id:string;selected_variant_id:string;recommended_variant_id:string|null;fit_preference:FitPreference|null;risk_type:RiskResult["risk"];risk_level:RiskResult["level"];evidence:{message?:string;usualSize?:Size|null;selectionContext?:{preference:string;label:string;answers?:Record<string,string>}};message:string;rule_set_id:string};
type Rule = {high_return_rate:number;minimum_improvement:number;low_stock_quantity:number;minimum_sample_size:number};
const rationaleByRisk:Record<RiskResult["risk"],AgentAnswer["rationaleCode"]>={size:"size_context",quality:"quality_context",preference_mismatch:"preference_context",insufficient_evidence:"insufficient_sample",none:"no_risk"};
...
export async function POST(request:Request){
  try{
    ...
    const result=intervention.evidence.selectionContext ? evaluateSelectionContext(product,selected,intervention.evidence.selectionContext) : evaluateCheckoutRisk(product,selected,intervention.fit_preference??"regular",thresholds,intervention.evidence.usualSize);
    ...
    const context={interventionId:intervention.id,product,selected,fitPreference:intervention.fit_preference??"regular",thresholds,deterministicResult:result,selectionContext:intervention.evidence.selectionContext};
