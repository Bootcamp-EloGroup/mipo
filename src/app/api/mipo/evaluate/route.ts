...
export async function POST(request: Request) {
  try {
    const body = await request.json(); const fitPreference = body.fitPreference ?? null; const usualSize = body.usualSize ?? null; const selectionContext = body.selectionContext && typeof body.selectionContext.preference === "string" && typeof body.selectionContext.label === "string" ? body.selectionContext as SelectionContext : null;
    ...
    const isApparel = product.productKind === "apparel" || product.variantAttribute === "size";
    ...
    const result = selectionContext ? evaluateSelectionContext(product, variant, selectionContext) : evaluateCheckoutRisk(product, variant, fitPreference ?? "regular", undefined, usualSize as Size | null);
    const response = NextResponse.json({ interventionId, result }); if (session.created) response.cookies.set(sessionCookie(session.id)); return response;
