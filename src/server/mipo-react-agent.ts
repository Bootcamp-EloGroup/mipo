...
function stepMessages(context:AgentContext, expectedTurn:string, observations:Array<{tool:string;observation:unknown}>):ProviderMessage[]{
  return [{role:"system",content:system},{role:"user",content:JSON.stringify({
    task:"Use somente as observações fornecidas e produza a próxima etapa esperada.",
    interventionId:context.interventionId,
    expectedTurn,
    completedTools:observations.map((item)=>item.tool),
    observations,
  })}];
}
...
export async function runMipoAgent(context: AgentContext): Promise<AgentAnswer> {
  ...
  const observations:Array<{tool:string;observation:unknown}>=[];
  let eloAvailable=true; let finalStatus:AgentAnswer["status"]="deterministic_fallback"; let rejection:string|null=null; let policyRejected=false; let lastModel="none"; const started=Date.now();
  try {
    for(let step=1;step<=MAX_STEPS;step++){
      ...
      if(turn?.type==="tool_call") { if(turn.tool!==TOOL_SEQUENCE[step-1])throw new Error("invalid_tool_sequence"); const observation=runTool(turn.tool,context); await persistStep(run.id,step,attempt,provider,"tool_call",turn.tool,"succeeded",response.latencyMs,observation); observations.push({tool:turn.tool,observation}); continue; }
      if(step!==MAX_STEPS)throw new Error("premature_final_answer");
      if(!turn)throw new Error("empty_agent_turn");
      rejection=validateFinalAnswer(turn,context); policyRejected=Boolean(rejection); await persistStep(run.id,step,attempt,provider,"final_answer",null,rejection?"rejected":"succeeded",response.latencyMs,{action:turn.action,rationaleCode:turn.rationaleCode,rejection});
      if(rejection) break;
      finalStatus=provider==="eloagents"?"eloagents_succeeded":"groq_succeeded";
      const finalTurn=turn as Extract<AgentTurn,{type:"final_answer"}>;
      const answer:AgentAnswer={action:finalTurn.action,message:finalTurn.message.trim(),rationaleCode:finalTurn.rationaleCode,provider,model:response.model,status:finalStatus};
