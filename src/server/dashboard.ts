...
type AgentRun = {status:"pending"|"eloagents_succeeded"|"groq_succeeded"|"deterministic_fallback"|"rejected_by_policy"|"cache_hit";latency_ms:number|null};
...
export async function getDashboardData(): Promise<DashboardData> {
  if (dataSource() !== "supabase") return { generatedAt:new Date().toISOString(), historicalWindow:{}, minimumSampleSize:30, metrics:{interventions:0,actionable:0,decidedActionable:0,accepted:0,keptOriginal:0,pending:0,abandoned:0,acceptanceRate:null}, riskDistribution:[], trend:[], rows:[], simulator:{locked:true,reason:"Conecte o Supabase para observar intervenções reais."},agent:{total:0,eloagents:0,groq:0,deterministicFallback:0,rejected:0,cacheHits:0,averageLatencyMs:null} };
  const [interventions, decisions, products, variants, rules, oldest, newest, agentRuns] = await Promise.all([
    ...
    supabaseRest<AgentRun[]>("mipo_agent_runs?select=status,latency_ms&order=created_at.desc&limit=5000"),
  ]);
  ...
  const risks = (["size","quality","preference_mismatch","stock","none","insufficient_evidence"] as const).map((risk)=>({risk,count:rows.filter((row)=>row.risk===risk).length})).filter((item)=>item.count>0);
