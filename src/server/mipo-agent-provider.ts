...
export async function callProvider(name: ProviderName, messages: ProviderMessage[], timeoutMs: number, expected:ExpectedTurn="final_answer", allowedMessages?:readonly string[]): Promise<{content:string;model:string;latencyMs:number}> {
  ...
  try {
    const responseFormat=strictFormat(expected,allowedMessages);
