...
export async function callProvider(name: ProviderName, messages: ProviderMessage[], timeoutMs: number): Promise<{content:string;model:string;latencyMs:number}> {
  ...
  try {
    ...
    const response = await fetch(`${provider.baseUrl}/chat/completions`, { method:"POST", signal:controller.signal, headers:{ Authorization:`Bearer ${provider.apiKey}`, "Content-Type":"application/json" }, body:JSON.stringify({ model:provider.model, temperature:0.1, max_completion_tokens:512, ...(name==="groq"?{reasoning_effort:"low"}:{}), messages, response_format:responseFormat }) });
