...
type DbOrder = { id: string; source_order_key: string; ordered_at: string; channel: string | null; customer_state: string | null; actual_delivery_days: number | null };
...
export type RulerLookup = { rulersFor: (channel: string | null, uf: string | null) => DeliveryRulerSet; all: DeliveryRuler[] };
...
export async function getActiveWismoRuleSet(): Promise<WismoRuleSet> {
  if (dataSource() === "local") return { id: "local", version: "2026-09-20.v1", thresholds: { preparationDays: 2, criticalExtraDays: 2, highConfidenceSample: 30, mediumConfidenceSample: 5, escalationGraceDays: 0 } };
...
export async function getRulerMatrix(runId: string): Promise<RulerLookup> {
  ...
  return {
    all,
    rulersFor: (channel, uf) => {
      const region = uf ? regionByUf.get(uf) : undefined;
      return {
        channel: channel ? byScope.get(`channel:${channel}`) : undefined,
        state: uf ? byScope.get(`state:${uf}`) : undefined,
        region: region ? byScope.get(`region:${region}`) : undefined,
        national,
      };
    },
  };
...
export async function rebuildDeliveryAssessments(): Promise<{ processed: number; deliveredOnTime: number; deliveredLate: number; criticalBreaches: number; byScope: Record<string, number>; ruleVersion: string }> {
  ...
  const summary = { processed: 0, deliveredOnTime: 0, deliveredLate: 0, criticalBreaches: 0, byScope: { channel: 0, state: 0, region: 0, national: 0 } as Record<string, number>, ruleVersion: rule.version };

  for (let offset = 0; ; offset += 1000) {
    const page = await supabaseRest<DbOrder[]>(`orders?import_run_id=eq.${runId}&actual_delivery_days=not.is.null&select=id,source_order_key,ordered_at,channel,customer_state,actual_delivery_days&order=source_order_key.asc&limit=1000&offset=${offset}`);
    const rows = page.map((row) => {
      const order: WismoOrder = { orderKey: row.source_order_key, orderedAt: row.ordered_at, channel: row.channel, customerState: row.customer_state, actualDeliveryDays: row.actual_delivery_days };
      const status = evaluateDeliveryStatus(order, matrix.rulersFor(row.channel, row.customer_state), new Date(), rule.thresholds);
      summary.processed += 1;
