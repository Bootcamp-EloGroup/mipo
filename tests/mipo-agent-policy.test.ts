...
beforeAll(async()=>({allowedActions,parseAgentTurn,validateFinalAnswer}=await import("../src/server/mipo-agent-policy")));
...
describe("política do agente ReAct",()=>{
  ...
  it("aceita mensagem curta dentro da ação autorizada",()=>{
    const context=contextFor(0,1,100);
    expect(validateFinalAnswer({type:"final_answer",action:"present_authorized_alternative",message:"Há uma alternativa de tamanho sustentada pelo histórico observado.",rationaleCode:"size_context"},context)).toBeNull();
  });
