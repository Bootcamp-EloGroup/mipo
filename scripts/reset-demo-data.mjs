...
const seedVersion=process.argv.includes("--seed")?process.argv[process.argv.indexOf("--seed")+1]:null;
async function reset(applyReset){const response=await fetch(`${baseUrl}/rest/v1/rpc/reset_demo_data`,{method:"POST",headers:{apikey:secret,Authorization:`Bearer ${secret}`,"Content-Type":"application/json"},body:JSON.stringify({apply_reset:applyReset,seed_version:seedVersion})});if(!response.ok)throw new Error(`${response.status}: ${await response.text()}`);return response.json();}
